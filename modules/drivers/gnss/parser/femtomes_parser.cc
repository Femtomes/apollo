...
// An parser for decoding binary messages from a Femtomes receiver. The following
// messages must be
...
#include "modules/drivers/gnss/parser/femtomes_messages.h"
...
// The Femtomes's orientation covariance matrix is pitch, roll, and yaw. We use
...
// CRC algorithm from the Femtomes document.
...
// Converts Femtomes's azimuth (north = 0, east = 90) to FLU yaw (east = 0, north
// = pi/2).
...
class FemtomesParser : public Parser {
 public:
  FemtomesParser();
  explicit FemtomesParser(const config::Config& config);
 ...
 private:
  ...
  bool HandleBestPos(const femtomes::BestPos* pos, uint16_t gps_week,
                     uint32_t gps_millisecs);

  bool HandleGnssBestpos(const femtomes::BestPos* pos, uint16_t gps_week,
                         uint32_t gps_millisecs);

  bool HandleBestVel(const femtomes::BestVel* vel, uint16_t gps_week,
                     uint32_t gps_millisecs);

  bool HandleCorrImuData(const femtomes::CorrImuData* imu);

  bool HandleInsPva(const femtomes::InsPva* pva);

  bool HandleInsPvax(const femtomes::InsPvaX* pvax, uint16_t gps_week,
                     uint32_t gps_millisecs);

  bool HandleRawImuX(const femtomes::RawImuX* imu);

  bool HandleRawImu(const femtomes::RawImu* imu);

  bool HandleBdsEph(const femtomes::BDS_Ephemeris* bds_emph);

  bool HandleGpsEph(const femtomes::GPS_Ephemeris* gps_emph);

  bool HandleGloEph(const femtomes::GLO_Ephemeris* glo_emph);
  ...
  bool HandleHeading(const femtomes::Heading* heading, uint16_t gps_week,
                     uint32_t gps_millisecs);
  ...
  femtomes::SolutionStatus solution_status_ =
      static_cast<femtomes::SolutionStatus>(femtomes::SolutionStatus::NONE);
  femtomes::SolutionType position_type_ =
      static_cast<femtomes::SolutionType>(femtomes::SolutionType::NONE);
  femtomes::SolutionType velocity_type_ =
      static_cast<femtomes::SolutionType>(femtomes::SolutionType::NONE);
  femtomes::InsStatus ins_status_ =
      static_cast<femtomes::InsStatus>(femtomes::InsStatus::NONE);
...
Parser* Parser::CreateFemtomes(const config::Config& config) {
  return new FemtomesParser(config);
}

FemtomesParser::FemtomesParser() {
...
FemtomesParser::FemtomesParser(const config::Config& config) {
  buffer_.reserve(BUFFER_SIZE);
  ins_.mutable_position_covariance()->Resize(9, FLOAT_NAN);
  ins_.mutable_euler_angles_covariance()->Resize(9, FLOAT_NAN);
  ins_.mutable_linear_velocity_covariance()->Resize(9, FLOAT_NAN);
...
Parser::MessageType FemtomesParser::GetMessage(MessagePtr* message_ptr) {
  if (data_ == nullptr) {
  ...
  while (data_ < data_end_) {
    if (buffer_.empty()) {  // Looking for SYNC0
      ...
      switch (*data_) {
        case femtomes::SYNC_2_LONG_HEADER:
          buffer_.push_back(*data_++);
          header_length_ = sizeof(femtomes::LongHeader);
          break;
        case femtomes::SYNC_2_SHORT_HEADER:
          buffer_.push_back(*data_++);
          header_length_ = sizeof(femtomes::ShortHeader);
          break;
        default:
          buffer_.clear();
      }
      ...
      if (buffer_.size() < header_length_) {
        ...
        if (header_length_ == sizeof(femtomes::LongHeader)) {
          total_length_ = header_length_ + femtomes::CRC_LENGTH +
                          reinterpret_cast<femtomes::LongHeader*>(buffer_.data())
                              ->message_length;
        } else if (header_length_ == sizeof(femtomes::ShortHeader)) {
          total_length_ =
              header_length_ + femtomes::CRC_LENGTH +
              reinterpret_cast<femtomes::ShortHeader*>(buffer_.data())
                  ->message_length;
        } else {
...
bool FemtomesParser::check_crc() {
  size_t l = buffer_.size() - femtomes::CRC_LENGTH;
  return crc32_block(buffer_.data(), l) ==
         *reinterpret_cast<uint32_t*>(buffer_.data() + l);
}

Parser::MessageType FemtomesParser::PrepareMessage(MessagePtr* message_ptr) {
  if (!check_crc()) {
  ...
  femtomes::MessageId message_id;
  uint16_t message_length;
  uint16_t gps_week;
  uint32_t gps_millisecs;
  if (buffer_[2] == femtomes::SYNC_2_LONG_HEADER) {
    auto header = reinterpret_cast<const femtomes::LongHeader*>(buffer_.data());
    message = buffer_.data() + sizeof(femtomes::LongHeader);
    gps_week = header->gps_week;
    gps_millisecs = header->gps_millisecs;
    message_id = header->message_id;
    message_length = header->message_length;
  } else {
    auto header = reinterpret_cast<const femtomes::ShortHeader*>(buffer_.data());
    message = buffer_.data() + sizeof(femtomes::ShortHeader);
    gps_week = header->gps_week;
    gps_millisecs = header->gps_millisecs;
    message_id = header->message_id;
    message_length = header->message_length;
  ...
  switch (message_id) {
    case femtomes::BESTGNSSPOS:
      if (message_length != sizeof(femtomes::BestPos)) {
        AERROR << "Incorrect message_length";
        break;
      }
      if (HandleGnssBestpos(reinterpret_cast<femtomes::BestPos*>(message),
    ...
    case femtomes::BESTPOS:
    case femtomes::PSRPOS:
      if (message_length != sizeof(femtomes::BestPos)) {
        AERROR << "Incorrect message_length";
        break;
      }
      if (HandleBestPos(reinterpret_cast<femtomes::BestPos*>(message), gps_week,
                        gps_millisecs)) {
        *message_ptr = &gnss_;
    ...
    case femtomes::BESTGNSSVEL:
    case femtomes::BESTVEL:
    case femtomes::PSRVEL:
      if (message_length != sizeof(femtomes::BestVel)) {
        AERROR << "Incorrect message_length";
        break;
      }
      if (HandleBestVel(reinterpret_cast<femtomes::BestVel*>(message), gps_week,
                        gps_millisecs)) {
        *message_ptr = &gnss_;
        return MessageType::GNSS;
      }
    ...
    case femtomes::INSPVAS:
      if (message_length != sizeof(femtomes::InsPva)) {
        AERROR << "Incorrect message_length";
        break;
      }

      if (HandleInsPva(reinterpret_cast<femtomes::InsPva*>(message))) {
        *message_ptr = &ins_;
        return MessageType::INS;
      }
      break;

    case femtomes::RAWIMUX:
    case femtomes::RAWIMUSX:
      if (message_length != sizeof(femtomes::RawImuX)) {
        AERROR << "Incorrect message_length";
        break;
      }

      if (HandleRawImuX(reinterpret_cast<femtomes::RawImuX*>(message))) {
        *message_ptr = &imu_;
        return MessageType::IMU;
      }
      break;

    case femtomes::RAWIMU:
    case femtomes::RAWIMUS:
      if (message_length != sizeof(femtomes::RawImu)) {
        AERROR << "Incorrect message_length";
        break;
      }

      if (HandleRawImu(reinterpret_cast<femtomes::RawImu*>(message))) {
        *message_ptr = &imu_;
        return MessageType::IMU;
      }
      break;

    case femtomes::INSPVAX:
      if (message_length != sizeof(femtomes::InsPvaX)) {
        AERROR << "Incorrect message_length";
        break;
      }

      if (HandleInsPvax(reinterpret_cast<femtomes::InsPvaX*>(message), gps_week,
                        gps_millisecs)) {
    ...
    case femtomes::BDSEPHEMERIS:
      if (message_length != sizeof(femtomes::BDS_Ephemeris)) {
        AERROR << "Incorrect BDSEPHEMERIS message_length";
        break;
      }
      if (HandleBdsEph(reinterpret_cast<femtomes::BDS_Ephemeris*>(message))) {
        *message_ptr = &gnss_ephemeris_;
        return MessageType::BDSEPHEMERIDES;
      }
      break;

    case femtomes::GPSEPHEMERIS:
      if (message_length != sizeof(femtomes::GPS_Ephemeris)) {
        AERROR << "Incorrect GPSEPHEMERIS message_length";
        break;
      }
      if (HandleGpsEph(reinterpret_cast<femtomes::GPS_Ephemeris*>(message))) {
        *message_ptr = &gnss_ephemeris_;
        return MessageType::GPSEPHEMERIDES;
      }
      break;

    case femtomes::GLOEPHEMERIS:
      if (message_length != sizeof(femtomes::GLO_Ephemeris)) {
        AERROR << "Incorrect GLOEPHEMERIS message length";
        break;
      }
      if (HandleGloEph(reinterpret_cast<femtomes::GLO_Ephemeris*>(message))) {
        *message_ptr = &gnss_ephemeris_;
    ...
    case femtomes::HEADING:
      if (message_length != sizeof(femtomes::Heading)) {
        AERROR << "Incorrect message_length";
        break;
      }
      if (HandleHeading(reinterpret_cast<femtomes::Heading*>(message), gps_week,
                        gps_millisecs)) {
...
bool FemtomesParser::HandleGnssBestpos(const femtomes::BestPos* pos,
                                      uint16_t gps_week,
                                      uint32_t gps_millisecs) {
...
bool FemtomesParser::HandleBestPos(const femtomes::BestPos* pos,
  ...
  if (pos->solution_status == femtomes::SolutionStatus::SOL_COMPUTED) {
    gnss_.set_position_type(static_cast<uint32_t>(pos->position_type));
    switch (pos->position_type) {
      case femtomes::SolutionType::SINGLE:
      case femtomes::SolutionType::INS_PSRSP:
        gnss_.set_type(apollo::drivers::gnss::Gnss::SINGLE);
        break;
      case femtomes::SolutionType::PSRDIFF:
      case femtomes::SolutionType::WAAS:
      case femtomes::SolutionType::INS_SBAS:
        gnss_.set_type(apollo::drivers::gnss::Gnss::PSRDIFF);
        break;
      case femtomes::SolutionType::FLOATCONV:
      case femtomes::SolutionType::L1_FLOAT:
      case femtomes::SolutionType::IONOFREE_FLOAT:
      case femtomes::SolutionType::NARROW_FLOAT:
      case femtomes::SolutionType::RTK_DIRECT_INS:
      case femtomes::SolutionType::INS_RTKFLOAT:
        gnss_.set_type(apollo::drivers::gnss::Gnss::RTK_FLOAT);
        break;
      case femtomes::SolutionType::WIDELANE:
      case femtomes::SolutionType::NARROWLANE:
      case femtomes::SolutionType::L1_INT:
      case femtomes::SolutionType::WIDE_INT:
      case femtomes::SolutionType::NARROW_INT:
      case femtomes::SolutionType::INS_RTKFIXED:
        gnss_.set_type(apollo::drivers::gnss::Gnss::RTK_INTEGER);
        break;
      case femtomes::SolutionType::OMNISTAR:
      case femtomes::SolutionType::INS_OMNISTAR:
      case femtomes::SolutionType::INS_OMNISTAR_HP:
      case femtomes::SolutionType::INS_OMNISTAR_XP:
      case femtomes::SolutionType::OMNISTAR_HP:
      case femtomes::SolutionType::OMNISTAR_XP:
      case femtomes::SolutionType::PPP_CONVERGING:
      case femtomes::SolutionType::PPP:
      case femtomes::SolutionType::INS_PPP_CONVERGING:
      case femtomes::SolutionType::INS_PPP:
        gnss_.set_type(apollo::drivers::gnss::Gnss::PPP);
        break;
      case femtomes::SolutionType::PROPOGATED:
  ...
  if (pos->datum_id != femtomes::DatumId::WGS84) {
...
bool FemtomesParser::HandleBestVel(const femtomes::BestVel* vel,
                                  uint16_t gps_week, uint32_t gps_millisecs) {
...
bool FemtomesParser::HandleCorrImuData(const femtomes::CorrImuData* imu) {
  rfu_to_flu(imu->x_velocity_change,
             imu->y_velocity_change,
             imu->z_velocity_change,
             ins_.mutable_linear_acceleration());
  rfu_to_flu(imu->x_angle_change,
             imu->y_angle_change,
             imu->z_angle_change,
...
bool FemtomesParser::HandleInsPva(const femtomes::InsPva* pva) {
  ...
  switch (pva->status) {
    case femtomes::InsStatus::ALIGNMENT_COMPLETE:
    case femtomes::InsStatus::SOLUTION_GOOD:
      ins_.set_type(apollo::drivers::gnss::Ins::GOOD);
      break;
    case femtomes::InsStatus::ALIGNING:
    case femtomes::InsStatus::HIGH_VARIANCE:
    case femtomes::InsStatus::SOLUTION_FREE:
...
bool FemtomesParser::HandleInsPvax(const femtomes::InsPvaX* pvax,
                                  uint16_t gps_week, uint32_t gps_millisecs) {
  double seconds = gps_week * SECONDS_PER_WEEK + gps_millisecs * 1e-3;
...
bool FemtomesParser::HandleRawImuX(const femtomes::RawImuX* imu) {
  if (imu->imu_error != 0) {
  ...
  if (is_zero(gyro_scale_)) {
    ...
    femtomes::ImuParameter param = femtomes::GetImuParameter(imu_type);
...
bool FemtomesParser::HandleRawImu(const femtomes::RawImu* imu) {
  double gyro_scale = 0.0;
  double accel_scale = 0.0;
  float imu_measurement_span = 1.0f / 200.0f;

  if (is_zero(gyro_scale_)) {
    femtomes::ImuParameter param = femtomes::GetImuParameter(imu_type_);
...
bool FemtomesParser::HandleGpsEph(const femtomes::GPS_Ephemeris* gps_emph) {
...
bool FemtomesParser::HandleBdsEph(const femtomes::BDS_Ephemeris* bds_emph) {
...
bool FemtomesParser::HandleGloEph(const femtomes::GLO_Ephemeris* glo_emph) {
...
bool FemtomesParser::HandleHeading(const femtomes::Heading* heading,
                                  uint16_t gps_week, uint32_t gps_millisecs) {
...
void FemtomesParser::SetObservationTime() {
  int week = 0;
...
bool FemtomesParser::DecodeGnssObservation(const uint8_t* obs_data,
                                          const uint8_t* obs_data_end) {
